...
The zip is **`manifestVersion` 1**: `manifest.json` plus the three CSVs (each file version 1). Historic PTI, `owner_id`, and app.adga.org Identity columns (empty until that scrape) are in that contract. See the design doc.
...
- `manifest.json` — format id, `manifestVersion` (1), exporter version, and per-file `version` + row counts.
