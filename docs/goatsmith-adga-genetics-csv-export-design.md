...
- Send a **batch** (individuals + LA rows + PTI rows) to the service worker, according to Settings (all on by default; user can opt out of ancestry stub rows, PTI, or Linear History). Sire and dam on the open animal are always recorded when visible.
